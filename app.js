...
(function () {
  ...
  function renderAlert() {
    ...
    const paint = () => {
      ...
      // 특보가 2개 이상이면 "(1/3)" 카운터 표시 → 순환 중임을 사용자에게 안내
      const counter = list.length > 1 ? ` <span class="ab-count">(${ALERT_INDEX + 1}/${list.length})</span>` : "";
      box.innerHTML =
        `<span class="ab-ico"><i data-lucide="alert-triangle"></i></span>
         <span class="ab-text"><b>${it.area || "서울"} [${it.title}]</b> 발효 중 — ${it.message || "안전에 유의하세요."}${counter}</span>`;
      refreshIcons();
    };
    paint();
